...
var TinyMCE_adsense_deluxe = {
	initInstance: function(inst) {
		tinyMCE.importCSS(inst.getDoc(), "/wp-content/plugins/adsense_deluxe/adsense_deluxe.css");
		tinyMCE.importCSS(inst.getDoc(), "/wp-content/mu-plugins/adsense_deluxe/adsense_deluxe.css");
	},
	...
	execCommand: function(editor_id, element, command, user_interface, value) {
		...
		switch (command) {
				...
				case "mce_ads_deluxe":
					...
					html = TinyMCE_adsense_deluxe.make_imgtag(value);
		...
		switch (type) {
			...
			case "insert_to_editor":
				...
				for(var i=0; i<ad_names.length; i++){
					...
					while ((startPos = content.indexOf('<!--'+ad_names[i]+'-->', startPos)) != -1) {
						...
						content += TinyMCE_adsense_deluxe.make_imgtag(ad_names[i]);
			...
			case "get_from_editor":
				...
				while ((startPos = content.indexOf('<img', startPos+1)) != -1) {
					...
					var attribs = TinyMCE_adsense_deluxe.parseAttributes(content.substring(startPos + 4, endPos));
...
tinyMCE.addPlugin("adsense_deluxe", TinyMCE_adsense_deluxe);
